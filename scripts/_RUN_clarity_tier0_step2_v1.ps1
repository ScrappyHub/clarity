...
function Get-LastMatchingLine {
  param([string[]]$Lines,[scriptblock]$Predicate)
  $matches = New-Object System.Collections.Generic.List[string]
  foreach($line in @($Lines)){ if(& $Predicate $line){ [void]$matches.Add($line) } }
...
$pkt = Get-LastMatchingLine -Lines $pktOut -Predicate { param($line) (Test-Path -LiteralPath $line -PathType Container) -and $line.StartsWith("C:\ProgramData\Clarity\outbox\", [StringComparison]::OrdinalIgnoreCase) }
...
$pledgeHash = Get-LastMatchingLine -Lines $pledgeOut -Predicate { param($line) $line -match "^[0-9a-f]{64}$" }
