...
# Replace KeyGen with corrected boolean expression (parenthesized Test-Path calls)
$keyGen =
  ...
  "  }" + "`r`n" +
  "  `$meta = [ordered]@{ clarity='Clarity Validator'; action='KEYGEN'; issued_at_utc=[DateTime]::UtcNow.ToString('o'); key_root=`$keyRoot; priv_path=`$kp.priv; pub_path=`$kp.pub; pub_sha256=(Sha256 `$kp.pub); backend=`$b.kind; mode='windows_userland_v1_4d' }" + "`r`n" +
...
Write-Host "PATCH OK (v1_4d)" -ForegroundColor Green
