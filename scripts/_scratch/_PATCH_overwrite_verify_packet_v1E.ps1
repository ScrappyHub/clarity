...
[void]$L.Add('')
[void]$L.Add('Write-Host "CLARITY_VERIFY_SENTINEL_V1E" -ForegroundColor DarkGray')
[void]$L.Add('function STEP([string]$s){ Write-Host ("STEP: " + $s) -ForegroundColor DarkGray }')
[void]$L.Add('')
[void]$L.Add('function Invoke-SshKeygenTimeout([string[]]$Args,[string]$StdinText,[int]$TimeoutMs){')
...
[void]$L.Add('  $psi.RedirectStandardInput  = $true')
...
[void]$L.Add('  if($StdinText -ne $null){')
[void]$L.Add('    $p.StandardInput.Write($StdinText)')
[void]$L.Add('  }')
[void]$L.Add('  $p.StandardInput.Close()')
...
[void]$L.Add('$stdin = ($commitHash + "`n" + $packetId + "`n" + $ingHash + "`n")')
[void]$L.Add('$args = @("-Y","verify","-f","`"" + $Allowed + "`"","-I","`"" + $Principal + "`"","-n","nfl.ingest.v1","-s","`"" + $sigPath + "`"")')
[void]$L.Add('$res = Invoke-SshKeygenTimeout $args $stdin 15000')
