...
## WBS 6 — Targeted integrity scanner — GREEN core (≈70%) — Step 6 done 2026-08-27

**Update:** `validator_scan_targeted.ps1` now consumes `clarity_rules.json` heuristics + an optional `clarity.baseline.v1` registry and emits the full `verified/unknown/suspicious/compromised` taxonomy plus `missing_critical`; report gains `rules_version/rules_hash`, `baseline_path/baseline_hash`, `classified_counts`. Proven by `SCANNER_BASELINE_TEST_OK` + regression-clean `HOST_SLICE_TEST_OK`. Sub-items 6.2/6.6/6.7 now GREEN; 6.3 baseline registry GREEN. Remaining: per-file signature validation (6.5), and the missing-critical → handoff wiring. Original breakdown below.
...
## WBS 7 — Isolation vault — GREEN core (≈85%) — Step 7 done 2026-08-27

**Update:** 7.8 execution-block (`isolation_block.ps1`: ReportOnly/Marker/critical-logical, non-destructive sidecar), 7.9 critical-file safety gate, 7.10 restoration pipeline (`isolation_restore.ps1`: authorized, object-identity-verified, hash-verified copy), and 7.11 restoration receipts (hash-chained restore + block ledgers) are now GREEN. Negative proofs added: unauthorized-restore-denied, corrupt-vault-object-rejected, restore-to-critical-denied, block-mutates-nothing. Proven by `CLARITY_TIER1_STEP7_OK` + `CLARITY_TIER1_STEP7B_OK`. Remaining: restore-target-state edge policies and richer authorization (token/multi-party). Original breakdown below.
...
## WBS 9 — Composed validator run + sealing — GREEN (≈95%) — Step 9.1 done 2026-08-27
